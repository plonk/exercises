...
int getop(char s[])
{
	...
	if (!isdigit(c) && c != '.' && c != '-') {
		/* わからない記号か、識別子の先頭 */
		if (isalpha(c)) {
			for (i = 1; isalnum(c = (pb != -1 ? tmp = pb, pb = -1, tmp : getch())); ++i) 
				s[i] = c;
			if (c != EOF)
				pb = c;
			s[i] = '\0';
			...
			/* さもなくば、文法エラー */
			if (isalpha(c = (pb != -1 ? tmp = pb, pb = -1, tmp : getch()))) {
				s[1] = c;
				s[2] = '\0';
				return ASSIGN;
			} else { /* EOF でも */
